...
for epoch in range(1, config.NUM_EPOCHS + 1):
    ...
    for i, data in enumerate(dataloader, start=0):
        ...
        reg_loss_netD = netD_historical_averaging(netD)
        lossD_total = lossD_real + lossD_fake + reg_loss_netD
        ...
        reg_loss_netG = netG_historical_averaging(netG)
        lossG_total = lossG + reg_loss_netG
        ...
        epoch_reg_loss_netG.append(reg_loss_netG)
        epoch_reg_loss_netD.append(reg_loss_netD)
        epoch_lossG_total.append(lossG_total.item())
        ...
        hist.save_iter_reg_loss_netD(iter_reg_loss_netD=reg_loss_netD)
        hist.save_iter_reg_loss_netG(iter_reg_loss_netG=reg_loss_netG)
        hist.save_iter_lossG_total(iter_lossG_total=lossG_total)
    ...
    hist.save_mean_epoch_lossG(mean_epoch_lossG=np.mean(epoch_lossG), epoch=epoch)
    ...
    hist.save_mean_epoch_reg_loss_netD(mean_epoch_reg_loss_netD=np.mean(epoch_reg_loss_netD), epoch=epoch)
    hist.save_mean_epoch_reg_loss_netG(mean_epoch_reg_loss_netG=np.mean(epoch_reg_loss_netG), epoch=epoch)
    hist.save_mean_epoch_lossG_total(mean_epoch_lossG_total=np.mean(epoch_lossG_total), epoch=epoch)
