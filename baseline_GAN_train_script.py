...
# Analytics
config.SAVE_NAME = 'baseline_test'
...
netG = baseline_GAN_net.BaselineGenerator(config).to(config.DEVICE)
# print('{}\n'.format(netG))

# Instantiate discriminator
netD = baseline_GAN_net.BaselineDiscriminator(config).to(config.DEVICE)
# print('{}\n'.format(netD))

# Load weights
# netG.apply(baseline_GAN_net.weights_init_normal)
# netD.apply(baseline_GAN_net.weights_init_normal)
...
for epoch in range(1, config.NUM_EPOCHS + 1):
    ...
    for i, data in enumerate(dataloader, start=0):
        ...
        # Define labels
        real_labels = torch.empty(size=(b_size,)).fill_(REAL_LABEL).to(config.DEVICE)
        fake_labels = torch.empty(size=(b_size,)).fill_(FAKE_LABEL).to(config.DEVICE)
