...
class GaussianNoise(nn.Module):
    ...
    Linear annealing
    """
    def __init__(self, sigma_init=0.3):
        super().__init__()
        self.iter_count = 0
        # self.register_buffer('noise_vector', torch.empty(0))
        self.register_buffer('noise_vector', torch.tensor(0))
        self.sigma_schedule = lambda iter_count: 0.001 + (sigma_init - 0.001) * math.exp(-1. * iter_count / 20000.)

    def forward(self, x):
        if self.training:
            sigma = self.sigma_schedule(self.iter_count)
            sampled_noise = self.noise_vector.repeat(*x.size()).float().normal_(mean=0, std=sigma) # TODO: * x.detach()
            x = x + sampled_noise
        return x
